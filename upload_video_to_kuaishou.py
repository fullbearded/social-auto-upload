...
if __name__ == '__main__':
    ...
    for index, file in enumerate(files):
        ...
        if thumbnail_path.exists():
            print(f"封面路径：{thumbnail_path}")
            print(f"封面路径类型：{type(thumbnail_path)}")
            print(f"即将创建KSVideo对象，thumbnail_path参数: {thumbnail_path}")
            app = KSVideo(title, file, tags, publish_datetimes[index], account_file, thumbnail_path=thumbnail_path)
            print(f"KSVideo对象创建完成，app.thumbnail_path: {app.thumbnail_path}")
        else:
            print(f"封面文件不存在: {thumbnail_path}")
